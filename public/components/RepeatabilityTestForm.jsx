...
const RepeatabilityTestForm = () => {
  ...
  const Emin = Math.min(...EcValues);
  const Emax = Math.max(...EcValues);
  const MIH = 1.0 * e; // örnek değer
  const uygun = Math.abs(Emax - Emin) <= MIH && EcValues.every(Ec => Math.abs(Ec) <= MIH);

  return (
    <div className="bg-white p-4 rounded shadow-md mt-8">
      <h2 className="text-lg font-semibold mb-4">2.3 - Tekrarlanabilirlik Testi</h2>
      ...
      <div className="mt-4 p-3 bg-gray-100 rounded">
        ...
        <p><strong>En Büyük Fark (Emax - Emin):</strong> {(Emax - Emin).toFixed(3)} kg</p>
...
export default RepeatabilityTestForm;
